...
class twitter{
	 ...
	 * @return string
	 */
	function isFriend( $target_user )
	{
	    if( !in_array( $this->type, array( 'xml','json' ) ) )
	        return false;
	        
		$qs = '?target_screen_name=' . rawurlencode( $target_user );
	...
	function followUser( $id, $notifications = false )
	{
		...
		$request = 'http://twitter.com/friendships/create.' . $this->type . '?screen_name=' . $id;
		if( $notifications )
		    $request .= '&follow=true';
	...
	function sendDirectMessage( $user, $text )
	{
        ...
        $postargs = 'screen_name=' . rawurlencode($user) . '&text=' . rawurlencode($text);
